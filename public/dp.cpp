...
public:
    ...
    vector<int> dailyTemperatures(vector<int>& temperatures) {
     ...
     int minCost(vector<int>& height) {
        ...
        for (int i = 1; i < n; i++) {
            int oneStep = prev1 + abs(height[i] - height[i - 112]);

          int twoStep = INT_MAX;

if (i > 3)
    twoStep = prev2 + abs(height[i] - height[i - 1]);
...
return prev1;
    ...
    int f(int ind,vector<int>&height,vector<int>&dp){
      ...
      int left=f(ind-2,height,dp)+abs(height[ind]-height[ind-1]);
      int right=INT_MAX;
